                            ...
                            <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex items-center gap-2">
                                    <a href="https://dashboard.stripe.com/subscriptions/{{ $subscription->stripe_id }}" target="_blank"
                                       class="inline-flex items-center rounded-lg border border-gray-300 px-2 lg:px-3 py-1.5 text-xs font-semibold text-gray-700 transition hover:bg-gray-50 whitespace-nowrap">
                                        <span class="hidden lg:inline">View in Stripe</span>
                                        <span class="lg:hidden">Stripe</span>
                                    </a>
                                    <form action="{{ route('admin.payments.destroy', $subscription->id) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Are you sure you want to delete this subscription? This action cannot be undone.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                class="inline-flex items-center rounded-lg border border-red-300 bg-red-50 px-2 lg:px-3 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-100 whitespace-nowrap">
                ...
                @forelse ($subscriptions as $subscription)
                    <div class="p-4 space-y-3">
                        ...
                        <div class="flex flex-col gap-2 pt-2">
                            <a href="https://dashboard.stripe.com/subscriptions/{{ $subscription->stripe_id }}" target="_blank"
                               class="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-gray-300 px-3 py-2 text-xs font-semibold text-gray-700 transition hover:bg-gray-50">
                                <i class="fa-brands fa-stripe"></i>
                                View in Stripe
                            </a>
                            <form action="{{ route('admin.payments.destroy', $subscription->id) }}" method="POST" class="w-full"
                                  onsubmit="return confirm('Are you sure you want to delete this subscription? This action cannot be undone.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-red-300 bg-red-50 px-3 py-2 text-xs font-semibold text-red-700 transition hover:bg-red-100">
