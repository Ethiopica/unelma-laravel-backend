<x-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 space-y-4 sm:space-y-6 lg:space-y-8">
        ...
        <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-900">Payments</h1>
                <p class="text-sm sm:text-base text-gray-600 mt-1">Manage payment subscriptions and one-time purchases.</p>
            </div>
        ...
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <a href="{{ route('admin.payments.index', ['type' => 'all'] + request()->except('type')) }}"
                    class="@if($type === 'all') border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                    All Payments
                </a>
                <a href="{{ route('admin.payments.index', ['type' => 'subscriptions'] + request()->except('type')) }}"
                    class="@if($type === 'subscriptions') border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                    Subscriptions
                ...
                <a href="{{ route('admin.payments.index', ['type' => 'purchases'] + request()->except('type')) }}"
                    class="@if($type === 'purchases') border-blue-500 text-blue-600 @else border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 @endif whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">
                    One-Time Payments
            ...
            @if(!empty($subscriptionSummary))
                <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4">
                    <div class="bg-white rounded-lg shadow-sm border border-blue-200 p-4 sm:p-5">
                        ...
                        <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ number_format($subscriptionSummary['total']) }}</p>
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-green-200 p-4 sm:p-5">
                        ...
                        <p class="text-2xl sm:text-3xl font-bold text-green-600">{{ number_format($subscriptionSummary['active']) }}</p>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-amber-200 p-4 sm:p-5">
                        ...
                        <p class="text-2xl sm:text-3xl font-bold text-amber-600">{{ number_format($subscriptionSummary['trialing']) }}</p>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-red-200 p-4 sm:p-5">
                        ...
                        <p class="text-2xl sm:text-3xl font-bold text-red-600">{{ number_format($subscriptionSummary['past_due']) }}</p>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5 col-span-2 md:col-span-1">
                        ...
                        <p class="text-2xl sm:text-3xl font-bold text-gray-600">{{ number_format($subscriptionSummary['canceled']) }}</p>
                    </div>
                </div>
            @endif
        @endif
        ...
        @if($type === 'all' || $type === 'purchases')
            @if(!empty($purchaseSummary))
                <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
                    <div class="bg-white rounded-lg shadow-sm border border-purple-200 p-4 sm:p-5">
                        <div class="flex items-center justify-between mb-2 sm:mb-3">
                            ...
                            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-purple-100 rounded-full flex items-center justify-center flex-shrink-0">
                                <i class="fa-solid fa-shopping-cart text-purple-600 text-sm sm:text-base"></i>
                            </div>
                        </div>
                        <p class="text-2xl sm:text-3xl font-bold text-gray-900">{{ number_format($purchaseSummary['total']) }}</p>
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-green-200 p-4 sm:p-5">
                        ...
                        <p class="text-2xl sm:text-3xl font-bold text-green-600">{{ number_format($purchaseSummary['completed']) }}</p>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-orange-200 p-4 sm:p-5">
                        <div class="flex items-center justify-between mb-2 sm:mb-3">
                            <p class="text-xs font-semibold uppercase text-gray-500">Refunded</p>
                            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-orange-100 rounded-full flex items-center justify-center flex-shrink-0">
                                <i class="fa-solid fa-undo text-orange-600 text-sm sm:text-base"></i>
                            </div>
                        </div>
                        <p class="text-2xl sm:text-3xl font-bold text-orange-600">{{ number_format($purchaseSummary['refunded']) }}</p>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-red-200 p-4 sm:p-5">
                        <div class="flex items-center justify-between mb-2 sm:mb-3">
                            <p class="text-xs font-semibold uppercase text-gray-500">Failed</p>
                            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0">
                                <i class="fa-solid fa-times-circle text-red-600 text-sm sm:text-base"></i>
                            </div>
                        </div>
                        <p class="text-2xl sm:text-3xl font-bold text-red-600">{{ number_format($purchaseSummary['failed']) }}</p>
                    </div>
                    <div class="bg-white rounded-lg shadow-sm border border-emerald-200 p-4 sm:p-5 col-span-2 md:col-span-1">
                        <div class="flex items-center justify-between mb-2 sm:mb-3">
                            <p class="text-xs font-semibold uppercase text-gray-500">Total Revenue</p>
                            <div class="w-10 h-10 sm:w-12 sm:h-12 bg-emerald-100 rounded-full flex items-center justify-center flex-shrink-0">
                                <i class="fa-solid fa-dollar-sign text-emerald-600 text-sm sm:text-base"></i>
                            </div>
                        </div>
                        <p class="text-2xl sm:text-3xl font-bold text-emerald-600">${{ number_format($totalRevenue ?? 0, 2) }}</p>
                        <p class="text-xs text-gray-500 mt-1">Subscriptions + Purchases</p>
                    </div>
                </div>
            @endif
        @endif
                ...
                <div class="px-4 sm:px-6 py-4 border-b border-gray-100">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h2 class="text-base sm:text-lg font-semibold text-gray-900">Subscriptions</h2>
                                <p class="text-xs sm:text-sm text-gray-500 hidden sm:block">Recurring payment subscriptions.</p>
                            </div>
                        </div>

                        <form method="GET" action="{{ route('admin.payments.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Paid</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($subscriptions as $subscription)
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-sm text-gray-600 truncate">{{ $subscription->user?->email ?? 'N/A' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-medium text-gray-900">{{ $subscription->name }}</div>
                                        <div class="text-sm text-gray-600 truncate max-w-xs">{{ $subscription->stripe_price }}</div>
                                    </td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        <span @class([
                                    ...
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $subscription->quantity ?? '—' }}</td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ optional($subscription->trial_ends_at)->format('M j, Y') ?? '—' }}</td>
                                ...
                                <tr>
                                    <td colspan="8" class="px-4 lg:px-6 py-10 text-center text-sm text-gray-500">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-sm text-gray-500 max-w-sm px-4">Once customers start subscribing, their payment details will show up here.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                        ...
                        <div class="p-4 space-y-3 bg-white">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-gray-900 truncate">{{ $subscription->user?->name ?? 'Unknown User' }}</p>
                                    <p class="text-xs text-gray-600 truncate">{{ $subscription->user?->email ?? 'N/A' }}</p>
                                </div>
                                <span @class([
                            ...
                            <div class="flex flex-col gap-2 pt-2">
                                ...
                                <form action="{{ route('admin.payments.destroy', $subscription->id) }}?type=subscription" method="POST" class="w-full"
                                    onsubmit="return confirm('Are you sure you want to delete this subscription? This action cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
        ...
        @if(($type === 'all' || $type === 'purchases') && $purchases)
            <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                <div class="px-4 sm:px-6 py-4 border-b border-gray-100">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="flex items-center gap-3">
                            <div class="inline-flex items-center justify-center rounded-full bg-purple-100 text-purple-600 w-8 h-8 sm:w-10 sm:h-10 flex-shrink-0">
                                <i class="fa-solid fa-shopping-cart text-sm sm:text-base"></i>
                            </div>
                            <div>
                                <h2 class="text-base sm:text-lg font-semibold text-gray-900">One-Time Payments</h2>
                                <p class="text-xs sm:text-sm text-gray-500 hidden sm:block">Single payment purchases.</p>
                            </div>
                        </div>

                        <form method="GET" action="{{ route('admin.payments.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                            <input type="hidden" name="type" value="{{ $type }}">
                            <label for="purchase_status" class="sr-only">Status</label>
                ...
                <div class="hidden md:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Customer</th>
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Item</th>
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Status</th>
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Paid</th>
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Quantity</th>
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Purchased</th>
                                <th scope="col" class="px-4 lg:px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                <p class="text-sm font-semibold text-gray-900 truncate">{{ $purchase->user?->name ?? 'Unknown User' }}</p>
                                                <p class="text-sm text-gray-600 truncate">{{ $purchase->user?->email ?? 'N/A' }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-600 truncate max-w-xs">{{ $purchase->stripe_price_id }}</div>
                                    </td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        <span @class([
                                            'inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold capitalize border',
                                    ...
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $purchase->quantity }}</td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                ...
                                <tr>
                                    <td colspan="7" class="px-4 lg:px-6 py-10 text-center text-sm text-gray-500">
                                        <div class="flex flex-col items-center gap-3">
                                            ...
                                            <p class="text-sm text-gray-500 max-w-sm px-4">Once customers make one-time payments, their purchase details will show up here.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                    ...
                    @forelse ($purchases as $purchase)
                        <div class="p-4 space-y-3 bg-white">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-semibold text-gray-900 truncate">{{ $purchase->user?->name ?? 'Unknown User' }}</p>
                                    <p class="text-xs text-gray-600 truncate">{{ $purchase->user?->email ?? 'N/A' }}</p>
                                </div>
                                <span @class([
                                    'inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold capitalize flex-shrink-0 border',
                                    'bg-green-100 text-green-800 border-green-200' => $purchase->status === 'completed',
                            ...
                            <div class="grid grid-cols-3 gap-3 text-sm">
                                ...
                                <div>
                                    <p class="text-xs text-gray-500 mb-1">Paid</p>
                                    <p class="font-medium text-gray-900">${{ number_format($purchase->amount, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-500 mb-1">Quantity</p>
                                    <p class="font-medium text-gray-900">{{ $purchase->quantity }}</p>
                                </div>
                            </div>
                            <div class="flex flex-col gap-2 pt-2">
                                @if($purchase->stripe_payment_intent_id)
                                ...
                                @endif
                                <form action="{{ route('admin.payments.destroy', $purchase->id) }}?type=purchase" method="POST" class="w-full"
                                    onsubmit="return confirm('Are you sure you want to delete this purchase? This action cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
