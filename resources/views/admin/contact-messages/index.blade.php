    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 space-y-4 sm:space-y-6 lg:space-y-8">
        ...
        <div class="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
            <div>
                ...
                <p class="text-sm sm:text-base text-gray-600 mt-1">View and manage customer contact messages and
                    inquiries</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-5">
                ...
                <p class="mt-2 sm:mt-3 text-2xl sm:text-3xl font-bold text-gray-900">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-red-200 p-4 sm:p-5">
                ...
                <p class="mt-2 sm:mt-3 text-2xl sm:text-3xl font-bold text-red-600">
                    {{ number_format($stats['unread_messages']) }}</p>
            </div>
            <div
                class="bg-white rounded-lg shadow-sm border border-green-200 p-4 sm:p-5 col-span-2 md:col-span-1">
                ...
                <p class="mt-2 sm:mt-3 text-2xl sm:text-3xl font-bold text-green-600">
                    {{ number_format($stats['total_messages'] - $stats['unread_messages']) }}</p>
            </div>
        </div>
                ...
                <div
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs sm:text-sm text-gray-500 hidden sm:block">Customer inquiries and
                                contact form submissions.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col"
                                    ...
                                    Date</th>
                                <th scope="col"
                                    class="px-4 lg:px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">
                                    Actions</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($messages as $message)
                                <tr
                                    class="hover:bg-gray-50 transition {{ !$message->is_read ? 'bg-blue-50' : '' }}">
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if (!$message->is_read)
                                                <span
                                                    class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-blue-100 text-blue-700">
                                                    New
                                                </span>
                                            @endif
                                        </div>
                                    ...
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm font-semibold text-gray-900">{{ $message->name }}
                                        </div>
                                    </td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap">
                                        @if ($message->is_read)
                                            <span
                                                class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold bg-green-100 text-green-700">
                                                Read
                                            </span>
                                        @else
                                            <span
                                    ...
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <span
                                            class="hidden lg:inline">{{ $message->created_at->format('M j, Y H:i') }}</span>
                                        <span
                                            class="lg:hidden">{{ $message->created_at->format('M j') }}</span>
                                    </td>
                                    <td class="px-4 lg:px-6 py-4 whitespace-nowrap text-sm">
                                        <div class="flex items-center justify-end gap-2">
                                            ...
                                            @if (!$message->is_read)
                                                <form
                                                    action="{{ route('admin.contact-messages.read', $message->id) }}"
                                            ...
                                            <form
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="delete-message-btn inline-flex items-center rounded-lg border border-red-200 px-2 lg:px-3 py-1.5 text-xs font-semibold text-red-600 transition hover:bg-red-50 whitespace-nowrap">
                        ...
                        <div class="p-4 space-y-3 {{ !$message->is_read ? 'bg-blue-50' : '' }}">
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-xs text-gray-500">
                                                {{ $message->created_at->format('M j, Y') }} · <span
                                        ...
                                        @else
                                            <span
                                                class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold bg-red-100 text-red-700 flex-shrink-0">
                                                Unread
                                            </span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-2 pt-2">
                                        ...
                                        <form action="{{ route('admin.contact-messages.destroy', $message->id) }}"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="delete-message-btn w-full inline-flex items-center justify-center gap-2 rounded-lg border border-red-200 px-3 py-2 text-xs font-semibold text-red-600 transition hover:bg-red-50">
