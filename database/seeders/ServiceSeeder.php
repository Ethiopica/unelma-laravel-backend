<?php

namespace Database\Seeders;

use App\Models\Service;
use Illuminate\Database\Seeder;

class ServiceSeeder extends Seeder
{
    /**
     * Run the database seeds.
     */
    public function run(): void
    {
        $services = [
            [
                'name' => 'Web Development',
                'description' => 'Building responsive and modern websites with latest technologies.',
                'icon' => 'fa-code',
                'image' => 'web_development.jpg',
                'image_url' => 'https://example.com/images/web_development.jpg',
                'is_active' => true,
                'is_featured' => true,
                'order' => 1,
                'created_at' => '2025-11-01 09:00:00',
                'updated_at' => '2025-11-01 09:00:00',
            ],
            [
                'name' => 'Mobile App Development',
                'description' => 'Creating cross-platform and native mobile applications.',
                'icon' => 'fa-mobile',
                'image' => 'mobile_app.jpg',
                'image_url' => 'https://example.com/images/mobile_app.jpg',
                'is_active' => true,
                'is_featured' => true,
                'order' => 2,
                'created_at' => '2025-11-02 10:15:00',
                'updated_at' => '2025-11-02 10:15:00',
            ],
            [
                'name' => 'UI/UX Design',
                'description' => 'Designing intuitive interfaces and user experiences.',
                'icon' => 'fa-pencil-ruler',
                'image' => 'ui_ux.jpg',
                'image_url' => 'https://example.com/images/ui_ux.jpg',
                'is_active' => true,
                'is_featured' => false,
                'order' => 3,
                'created_at' => '2025-11-03 11:30:00',
                'updated_at' => '2025-11-03 11:30:00',
            ],
            [
                'name' => 'SEO Optimization',
                'description' => 'Improving website ranking and visibility on search engines.',
                'icon' => 'fa-search',
                'image' => 'seo.jpg',
                'image_url' => 'https://example.com/images/seo.jpg',
                'is_active' => true,
                'is_featured' => true,
                'order' => 4,
                'created_at' => '2025-11-04 12:45:00',
                'updated_at' => '2025-11-04 12:45:00',
            ],
            [
                'name' => 'Cloud Hosting',
                'description' => 'Reliable and scalable cloud hosting services.',
                'icon' => 'fa-cloud',
                'image' => 'cloud_hosting.jpg',
                'image_url' => 'https://example.com/images/cloud_hosting.jpg',
                'is_active' => true,
                'is_featured' => false,
                'order' => 5,
                'created_at' => '2025-11-05 14:00:00',
                'updated_at' => '2025-11-05 14:00:00',
            ],
            [
                'name' => 'Digital Marketing',
                'description' => 'Promoting brands through digital channels effectively.',
                'icon' => 'fa-bullhorn',
                'image' => 'digital_marketing.jpg',
                'image_url' => 'https://example.com/images/digital_marketing.jpg',
                'is_active' => true,
                'is_featured' => true,
                'order' => 6,
                'created_at' => '2025-11-06 15:15:00',
                'updated_at' => '2025-11-06 15:15:00',
            ],
            [
                'name' => 'Content Writing',
                'description' => 'High-quality content creation for websites and blogs.',
                'icon' => 'fa-pen',
                'image' => 'content_writing.jpg',
                'image_url' => 'https://example.com/images/content_writing.jpg',
                'is_active' => true,
                'is_featured' => false,
                'order' => 7,
                'created_at' => '2025-11-07 16:30:00',
                'updated_at' => '2025-11-07 16:30:00',
            ],
            [
                'name' => 'E-commerce Solutions',
                'description' => 'Developing scalable e-commerce platforms for businesses.',
                'icon' => 'fa-shopping-cart',
                'image' => 'ecommerce.jpg',
                'image_url' => 'https://example.com/images/ecommerce.jpg',
                'is_active' => true,
                'is_featured' => true,
                'order' => 8,
                'created_at' => '2025-11-08 17:45:00',
                'updated_at' => '2025-11-08 17:45:00',
            ],
            [
                'name' => 'IT Consulting',
                'description' => 'Providing expert IT consulting and strategy services.',
                'icon' => 'fa-lightbulb',
                'image' => 'it_consulting.jpg',
                'image_url' => 'https://example.com/images/it_consulting.jpg',
                'is_active' => true,
                'is_featured' => false,
                'order' => 9,
                'created_at' => '2025-11-09 18:00:00',
                'updated_at' => '2025-11-09 18:00:00',
            ],
            [
                'name' => 'Cybersecurity',
                'description' => 'Protecting systems and data from cyber threats.',
                'icon' => 'fa-shield-alt',
                'image' => 'cybersecurity.jpg',
                'image_url' => 'https://example.com/images/cybersecurity.jpg',
                'is_active' => true,
                'is_featured' => true,
                'order' => 10,
                'created_at' => '2025-11-10 19:15:00',
                'updated_at' => '2025-11-10 19:15:00',
            ],
        ];

        Service::insert($services);
    }
}
